...
package com.example.android.autoeditor.tracking;
...
public class ObjectTracker {
  ...
  private static class TimestampedDeltas {
    ...
    TimestampedDeltas(final long timestamp, final byte[] deltas) {
  ...
  public static class Keypoint {
    ...
    Keypoint(final float x, final float y) {
      this.x = x;
      this.y = y;
      this.score = 0;
      this.type = -1;
    ...
    Keypoint(final float x, final float y, final float score, final int type) {
      this.x = x;
      this.y = y;
      this.score = score;
      this.type = type;
  ...
  public static class PointChange {
    final Keypoint keypointA;
    final Keypoint keypointB;
    Keypoint pointDelta;
    private final boolean wasFound;

    PointChange(final float x1, final float y1,
  ...
  public static class FrameChange {
    static final int KEYPOINT_STEP = 7;

    final Vector<PointChange> pointDeltas;

    private final float minScore;
    private final float maxScore;

    FrameChange(final float[] framePoints) {
