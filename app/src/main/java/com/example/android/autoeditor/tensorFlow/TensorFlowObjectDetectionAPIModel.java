...
package com.example.android.autoeditor.tensorFlow;
...
public class TensorFlowObjectDetectionAPIModel implements Classifier {
    ...
    @Override
    public List<Entity> recognizeEntities(Bitmap bitmap) { //returns a list of recognized objects in the image
        ...
        final PriorityQueue<Entity> pq =
                new PriorityQueue<>(
                        1,
                        new Comparator<Entity>() {
                            @Override
                            public int compare(final Entity lhs, final Entity rhs) {
        ...
        for (int i = 0; i < outputScores.length; i++) {
            final RectF detectionLocation = new RectF(
            ...
            pq.add(new Entity(
                    "" + i,
                    labels.get((int) outputClasses[i]),
                    outputScores[i],
                    detectionLocation));
        }
        ...
        for (int i = 0; i < Math.min(pq.size(), MAX_RESULTS); ++i) {
            entities.add(pq.poll()); //returns null if empty
        }
